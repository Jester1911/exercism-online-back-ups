...
def compare_records(azara_record: tuple, rui_record: tuple) -> bool:
    ...
    # cannot use 'is' for comparison because I think tuples use references
    # for comparison. So == is shallow equality and I think === is deep
    # equality.
...
def clean_up(combined_record_group) -> str:
    ...
    str_output = "\n".join(
        f"('{record[0]}', '{record[2]}', {record[3]}, '{record[4]}')" for record in combined_record_group
    )
